...
if not app.debug:
    if app.config['MAIL_SERVER']:
        ...
        mail_handler = SMTPHandler(
            ...
            toaddrs=app.config['ADMINS'], subject='Message Wall Failure',
...
def follow(username):
    ...
    if user is None:
        #flash('User {} not found.'.format(username)) #l18 L10 not supported
    ...
    db.session.commit()
    #flash('You are following {}!'.format(username)) #l18 L10 not supported
...
def unfollow(username):
    ...
    db.session.commit()
    #flash('You are not following {}.'.format(username)) #l18 L10 not supported
